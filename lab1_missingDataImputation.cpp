...
int t=11, n=0, m=0, choice, avg_age, avg_salary;
string country[20];
string sAge[20]; int age[20];
string sSalary[20]; int salary[20];
string purchase[20];

void input(){
    ifstream inFile ( "Data.csv" ); // taking whole file in memory
    for(int i=1; i<=t; i++){
        //input each row
        getline(inFile, country[i], ',');
        getline(inFile,sAge[i],',');       if(sAge[i]!="NaN"){age[i]=atoi(sAge[i].c_str()); n++;}
        getline(inFile,sSalary[i],',');   if(sSalary[i]!="NaN"){salary[i]=atoi(sSalary[i].c_str()); m++;}
        getline(inFile, purchase[i], '\n');
    }
}

void showData(){
    for(int i=1; i<=t; i++)
        cout<<country[i]<<","<<sAge[i]<<","<<sSalary[i]<<","<<purchase[i]<<endl;
}

int average(int a[], int r){
    int sum=0;
    for(int i=1; i<=r; i++)
        sum = sum + a[i];
    int avg = sum/r;
